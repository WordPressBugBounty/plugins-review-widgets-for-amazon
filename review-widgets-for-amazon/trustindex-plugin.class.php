...
'name' => __('Free Widget Configurator', 'review-widgets-for-amazon')
];
if ($this->is_noreg_linked()) {
$tabs[] = [
'place' => 'left',
'slug' => 'my-reviews',
'name' => __('My reviews', 'review-widgets-for-amazon')
];
}
$tabs[] = [
'place' => 'left',
'slug' => 'get-reviews',
'name' => __('Get Reviews', 'review-widgets-for-amazon')
];
$tabs[] = [
'place' => 'left',
'slug' => 'rate-us',
'name' => __('Rate Us', 'review-widgets-for-amazon')
];
if (!$this->is_trustindex_connected()) {
$tabs[] = [
'place' => 'left',
'slug' => 'get-more-features',
'name' => __('Get more Features', 'review-widgets-for-amazon')
];
}
$tabs[] = [
'place' => 'right',
...
'place' => 'right',
'slug' => 'advanced',
'name' => __('Advanced', 'review-widgets-for-amazon')
];
return $tabs;
}
...
return '<a class="ti-btn" href="https://www.trustindex.io/?a=sys&c='. $campaignId .'" target="_blank">'. __('Create a Free Account for More Features', 'review-widgets-for-amazon') .'</a>';
}
...
$settingsPageTitle .= mb_strtolower(__('Reviews', 'review-widgets-for-amazon'));
}
else {
$settingsPageTitle .= strtolower(__('Reviews', 'review-widgets-for-amazon'));
}
$topMenu = false;
...
$platformLink .= 'href="' . admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php&tab=my-reviews') . '">'. __('Review Management', 'review-widgets-for-amazon');
}
else {
$platformLink .= 'href="' . admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php') . '">';
if (!$this->is_noreg_linked()) {
/* translators: %s: Platform name */
$platformLink .= sprintf(__('Connect %s', 'review-widgets-for-amazon'), $this->platform_name);
}
else {
$platformLink .= __('Create Widget', 'review-widgets-for-amazon');
}
}
$platformLink .= '</a>';
$settingsLink = '<a href="' . admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php') . '">' . __('Settings', 'review-widgets-for-amazon') . '</a>';
...
{
if (basename($file) === basename($this->plugin_file_path)) {
$meta[] = '<a href="'. admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php&tab=get-more-features') .'">'. __('Get more Features', 'review-widgets-for-amazon') . ' →</a>';
$meta[] = '<a href="http://wordpress.org/support/view/plugin-reviews/'. $this->get_plugin_slug() .'" target="_blank" rel="noopener noreferrer">'. __('Rate our plugin', 'review-widgets-for-amazon') . ' <span style="color: #F6BB07; font-size: 1.2em; line-height: 1; position: relative; top: 0.05em;">★★★★★</span></a>';
}
return $meta;
}
...
sprintf(__('We have worked a lot on the free "%s" plugin.', 'review-widgets-for-amazon'), $this->plugin_name) . '<br />' .
__('If you love our features, please write a review to help us make the plugin even better.', 'review-widgets-for-amazon') . '<br />' .
/* translators: %s: Trustindex CEO */
sprintf(__('Thank you. Gabor, %s', 'review-widgets-for-amazon'), 'Trustindex CEO'),
],
'not-using-no-connection' => [
'type' => 'warning',
'extra-class' => "",
/* translators: %s: Platform name */
'button-text' => sprintf(__('Create a free %s widget! »', 'review-widgets-for-amazon'), $platformName),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
...
/* translators: %s: Platform name */
'text' => sprintf(__('Display %s reviews on your website.', 'review-widgets-for-amazon'), $platformName),
],
'not-using-no-widget' => [
'type' => 'warning',
'extra-class' => "",
/* translators: %s: Platform name */
'button-text' => sprintf(__('Embed the %s reviews widget! »', 'review-widgets-for-amazon'), $platformName),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
...
'redirect' => $defaultRedirect,
/* translators: %s: Platform name */
'text' => sprintf(__('Build trust and display your %s reviews on your website.', 'review-widgets-for-amazon'), $platformName),
],
'review-download-available' => [
'type' => 'warning',
'extra-class' => "",
'button-text' => __('Download your latest reviews! »', 'review-widgets-for-amazon'),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
'remind-later-button' => false,
...
/* translators: %s: Platform name */
'text' => sprintf(__('You can update your %s reviews.', 'review-widgets-for-amazon'), $platformName),
],
'review-download-finished' => [
'type' => 'warning',
'extra-class' => "",
/* translators: %s: Service name (ChatGPT) */
'button-text' => sprintf(__('Reply with %s! »', 'review-widgets-for-amazon'), 'ChatGPT'),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
'remind-later-button' => false,
'redirect' => '?page='. $this->get_plugin_slug() .'/settings.php&tab=my-reviews',
/* translators: %s: Platform name */
'text' => sprintf(__('Your new %s reviews have been downloaded.', 'review-widgets-for-amazon'), $platformName),
],
];
...
.' data-ti-notice-dismiss-text="'. esc_attr(__('Dismiss', 'review-widgets-for-amazon')) .'"'
: "";
...
$chosedPlatform = new $className($forcePlatform, $filePath, "do-not-care-14.0", "do-not-care-Widgets for Amazon Reviews", "do-not-care-Amazon");
$chosedPlatform->setNotificationParam('not-using-no-widget', 'active', false);
if (!$chosedPlatform->is_noreg_linked()) {
/* translators: %s: Platform name */
return wp_kses_post($this->frontEndErrorForAdmins(sprintf(__('You have to connect your business (%s)!', 'review-widgets-for-amazon'), $forcePlatform)));
...
return wp_kses_post($this->frontEndErrorForAdmins(__('Your shortcode is deficient: Trustindex Widget ID is empty! Example: ', 'review-widgets-for-amazon') . '<br /><code>['.$this->get_shortcode_name().' data-widget-id="478dcc2136263f2b3a3726ff"]</code>'));
}
}
...
if ('' === $title) {
$title = __('Trustindex plugin', 'review-widgets-for-amazon');
}
return self::get_alertbox($type, '<strong>'.$title.'</strong><br />'.$text.'<br /><i style="opacity: 0.65">('. sprintf(__('This message is not be visible to visitors in public mode.') .')</i>', 'review-widgets-for-amazon'), false, $dismissUrl);
}
...
<p>' .
'<strong>'. __('ERROR with the following plugin:', 'review-widgets-for-amazon') .'</strong> '. $this->plugin_name .'<br /><br />' .
__('CSS file could not saved.', 'review-widgets-for-amazon') .' <strong>('. $this->getCssFile() .')</strong> '. __('Your widgets do not display properly!', 'review-widgets-for-amazon') . '<br />';
...
if ($fileExists) {
$html .= __('CSS file exists and it is not writeable. Delete the file', 'review-widgets-for-amazon');
}
else {
$html .= __('Grant write permissions to upload folder', 'review-widgets-for-amazon');
}
$html .= '<br />' .
__('or', 'review-widgets-for-amazon') . '<br />' .
/* translators: %s: Advanced page link */
sprintf(__("enable 'CSS internal loading' in the %s page!", 'review-widgets-for-amazon'), '<a href="'. admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php&tab=advanced') .'>'. __('Advanced', 'review-widgets-for-amazon') .'</a>');
}
echo wp_kses($html, [
'p' => [], 'strong' => [], 'br' => [],
...
return wp_kses_post($this->frontEndErrorForAdmins(__('Free plugin features are unavailable with AMP plugin.', 'review-widgets-for-amazon')));
}
...
$text = sprintf(__('Our exclusive "Top Rated" badge is awarded to service providers with a rating of %s and above.', 'review-widgets-for-amazon'), self::$topRatedMinimumScore)
.'<br />'
.'<a href="'.admin_url('admin.php?page='.$this->get_plugin_slug().'/settings.php&tab=free-widget-configurator&step=2').'">'.__('Please select another widget', 'review-widgets-for-amazon').'.</a>';
return wp_kses_post($this->frontEndErrorForAdmins($text));
}
if ($reviews = $this->getReviewsForWidgetHtml()) {
$this->increaseViews();
$preContent = $this->getWidgetStylesheetHtml()
.$this->escapeWidgetHtml($this->prepareWidgetHtmlForFrontend($this->getWidgetHtml($reviews)));
} else {
/* translators: %s: Platform name */
$text = sprintf(__('There are no reviews on your %s platform.', 'review-widgets-for-amazon'), ucfirst($this->getShortName()));

if ($this->is_review_download_in_progress()) {
$text = __('Your reviews are being downloaded.', 'review-widgets-for-amazon') . ' ' . __('This process should only take a few minutes.', 'review-widgets-for-amazon');
}
return wp_kses_post($this->frontEndErrorForAdmins($text));
}
if ($this->is_trustindex_connected() && !$this->isNoticeDismissed('free-widget')) {
$title = __('You are still using the free widget below.', 'review-widgets-for-amazon');
$text = __('Switch to the Pro version by replacing the shortcode.', 'review-widgets-for-amazon')
.'<br /><a href="'.esc_url('https://admin.trustindex.io/widget').'" target="_blank" rel="noopener noreferrer">'.__('Find the shortcode in the widget list.', 'review-widgets-for-amazon').'</a>';
...
private static $widgetStylesheetRendered = false;
private function getWidgetStylesheetHtml()
{
if (self::$widgetStylesheetRendered) {
return "";
}
self::$widgetStylesheetRendered = true;
$handle = 'trustindex-'.$this->getShortName().'-widget';
if (is_file($this->getCssFile()) && !get_option($this->get_option_name('load-css-inline'), 0)) {
wp_enqueue_style($handle, $this->getCssUrl(), [], filemtime($this->getCssFile()));
} else {
wp_register_style($handle, false, [], $this->getVersion());
wp_enqueue_style($handle);
wp_add_inline_style($handle, get_option($this->get_option_name('css-content')));
}
ob_start();
wp_print_styles($handle);
return $this->addOptimizerOptOutAttributes(ob_get_clean());
}

private function addOptimizerOptOutAttributes($html)
{
...
return $this->getWidgetPresettedStylesheetHtml($html).$this->escapeWidgetHtml($this->prepareWidgetHtmlForFrontend($html));
}
...
private function getWidgetPresettedStylesheetHtml($html)
{
if (!preg_match('/data-layout-id="(\d+)"/', $html, $layoutMatch) || !preg_match('/data-set-id="([a-z0-9-]+)"/i', $html, $setMatch)) {
return "";
}
$fileName = $layoutMatch[1].'-'.$setMatch[1].'.css';
$version = preg_match('/data-css-version="(\d+)"/', $html, $versionMatch) ? 'v'.$versionMatch[1].'/' : "";
$handle = 'trustindex-widget-'.$layoutMatch[1].'-'.$setMatch[1];
wp_enqueue_style($handle, "https://cdn.trustindex.io/assets/widget-presetted-css/$version$fileName", [], true);
ob_start();
wp_print_styles($handle);
return $this->addOptimizerOptOutAttributes(ob_get_clean());
}
...
update_option($this->get_option_name('widget-html-'.$tiPublicId), $html, false);
...
return $this->frontEndErrorForAdmins(__('Could not download the template for the widget.<br />Please reload the page.<br />If the problem persists, please write an email to support@trustindex.io.', 'review-widgets-for-amazon') .'<br /><br />'. print_r($response, true));
}
...
if ($this->is_trustindex_connected()): ?>
<?php if ($ti_widgets): ?>
<h2><?php echo esc_html(__('Your saved widgets', 'review-widgets-for-amazon')); ?></h2>
...
esc_html(__("You have no widget saved!", 'review-widgets-for-amazon')) . " "
. "<a target='_blank' href='" . "https://admin.trustindex.io/" . "widget'>". esc_html(__("Let's go, create amazing widgets for free!", 'review-widgets-for-amazon'))."</a>"
)); ?>
<?php endif; ?>
<?php else: ?>
<?php echo wp_kses_post(self::get_alertbox("warning",
esc_html(__("You have not set up your Trustindex account yet!", 'review-widgets-for-amazon')) . " "
/* translators: %s: URL */
. wp_kses(sprintf(__("Go to <a href='%s'>plugin setup page</a> to complete the one-step setup guide and enjoy the full functionalization!", 'review-widgets-for-amazon'), admin_url('admin.php?page='.$this->get_plugin_slug().'/settings.php&tab=advanced')))
)); ?>
<?php endif;
wp_die();
}
