...
Plugin Title: Widgets for Amazon Reviews Plugin
Plugin URI: https://wordpress.org/plugins/review-widgets-for-amazon/
Description: Embed Amazon reviews fast and easily into your WordPress site. Increase SEO, trust and sales using Amazon reviews.
...
$trustindex_pm_amazon = new TrustindexPlugin_amazon("amazon", __FILE__, "14.2", "Widgets for Amazon Reviews", "Amazon");
$pluginManager = 'TrustindexPlugin_amazon';
$pluginManagerInstance = $trustindex_pm_amazon;
...
add_filter('style_loader_tag', function($tag, $handle) use($pluginManagerInstance) {
return 0 === strpos($handle, 'trustindex-widget-css-') ? $pluginManagerInstance->addOptimizerOptOutAttributes($tag) : $tag;
}, 10, 2);
...
require_once(__DIR__ . '/include/elementor-widgets.php');
$widgetsManager->register(new \Elementor\TrustrindexElementorWidget_Amazon([], [$pluginManagerInstance]));
}
});
...
require_once(__DIR__ . '/include/elementor-widgets.php');
$widgetsManager->register_widget_type(new \Elementor\TrustrindexElementorWidget_Amazon([], [$pluginManagerInstance]));
}
});
...
echo wp_kses_post(sprintf(__('Download our new <a href="%1$s" target="_blank">%2$s</a> plugin and get features for free!', 'review-widgets-for-amazon'), 'https://wordpress.org/plugins/customer-reviews-collector-for-woocommerce/', 'Customer Reviews Collector for WooCommerce'));
?></strong></p>
<ul style="list-style-type: disc; margin-left: 10px; padding-left: 15px">
<li><?php echo esc_html(__('Send unlimited review invitations for free', 'review-widgets-for-amazon')); ?></li>
<li><?php echo esc_html(__('E-mail templates are fully customizable', 'review-widgets-for-amazon')); ?></li>
<li><?php echo esc_html(__('Collect reviews on 100+ review platforms (Google, Facebook, Yelp, etc.)', 'review-widgets-for-amazon')); ?></li>
</ul>
<p>
<a href="<?php echo esc_url(wp_nonce_url(admin_url("admin.php?page=review-widgets-for-amazon/settings.php&wc_notification=open"), 'ti-wc-notification')); ?>" target="_blank" class="ti-close-notification" style="text-decoration: none">
<button class="button button-primary"><?php echo esc_html(__('Download plugin', 'review-widgets-for-amazon')); ?></button>
</a>
<a href="<?php echo esc_url(wp_nonce_url(admin_url("admin.php?page=review-widgets-for-amazon/settings.php&wc_notification=hide"), 'ti-wc-notification')); ?>"target="_blank" class="ti-hide-notification" style="text-decoration: none">
<button class="button button-secondary"><?php echo esc_html(__('Do not remind me again', 'review-widgets-for-amazon')); ?></button>
</a>
</p>
</div>
<?php
...
<button class="button ti-button-primary button-primary">'. esc_html(__('Write a review', 'review-widgets-for-amazon')) .'</button>
</a>
...
'. esc_html(__('Maybe later', 'review-widgets-for-amazon')) .'
</a>
...
'. esc_html(__('Do not remind me again', 'review-widgets-for-amazon')) .'
</a>
';
}
else {
...
'. esc_html(__('Remind me later', 'review-widgets-for-amazon')) .'
</a>';
}
}
echo '
